...
#   ifdef UP5_MASKING
                mask0=vmask(i,j-1)*vmask(i,j)
                IF (vmask(i,j-1).GT. 1.1 .or. vmask(i,j-1).LT. -1.1) THEN
                    print*, 'vmask(i,j-1)',vmask(i,j-1)
                ENDIF
                IF (vmask(i,j).GT. 1.1 .or. vmask(i,j).LT. -1.1) THEN 
                    print*, 'vmask(i,j)',vmask(i,j)
                ENDIF
                mask2=vmask(i,j-2)*mask0*vmask(i,j+1)
                IF (vmask(i,j-2).GT. 1.1 .or. vmask(i,j-2).LT. -1.1) THEN
                    print*, 'vmask(i,j-2)',vmask(i,j-2)
                ENDIF
                IF (vmask(i,j+1).GT. 1.1 .or. vmask(i,j+1).LT. -1.1) THEN
                    print*, 'vmask(i,j+1)',vmask(i,j+1)
                ENDIF
                IF (vel.gt.0) THEN
                  ...
                  IF (vmask(i,j-2).GT. 1.1 .or. vmask(i,j-2).LT. -1.1) THEN
                    print*, 'vmask(i,j-2)',vmask(i,j-2)
                ENDIF
                IF (vmask(i,j-3).GT. 1.1 .or. vmask(i,j-3).LT. -1.1) THEN
                    print*, 'vmask(i,j-3)',vmask(i,j-3)
                ENDIF
                ELSE
...
#  ifdef MASKING
     ...
     &             t(i-1,j,k,nrhs,itrc), t(i,j,k,nrhs,itrc), vel, cdif)
#   ifdef UP5_MASKING
                mask0=umask(i-1,j)*umask(i,j)
                IF (umask(i-1,j).GT. 1.1 .or. umask(i-1,j).LT. -1.1) THEN
                    print*, 'umask(i-1,j)',umask(i-1,j)
                ENDIF
                IF (umask(i,j).GT. 1.1 .or. umask(i,j).LT. -1.1) THEN
                    print*, 'umask(i,j)',umask(i,j)
                ENDIF
                mask2=umask(i-2,j)*mask0*umask(i+1,j)
                IF (umask(i-2,j).GT. 1.1 .or. umask(i-2,j).LT. -1.1) THEN
                    print*, 'umask(i-2,j)',umask(i-2,j)
                ENDIF
                IF (umask(i+1,j).GT. 1.1 .or. umask(i+1,j).LT. -1.1) THEN
                    print*, 'umask(i+1,j)',umask(i+1,j)
                ENDIF
                IF (vel.gt.0) THEN
                  ...
                  IF (umask(i-2,j).GT. 1.1 .or. umask(i-2,j).LT. -1.1) THEN
                    print*, 'umask(i-2,j)',umask(i-2,j)
                ENDIF
                IF (umask(i-3,j).GT. 1.1 .or. umask(i-3,j).LT. -1.1) THEN
                    print*, 'umask(i-3,j)',umask(i-3,j)
                ENDIF
                ELSE
